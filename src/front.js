...
var dom = {
  ...
  clone: function (element, value) {
    ...
    var from = element.call.subElement || app.element.select(element.call.value),
      to = element.call.element

    if (!from || !to) return
    var clone = from.cloneNode(true)
    clone.removeAttribute('id')
    to.innerHTML = to.getAttribute('inherit') === 'false' ? clone.innerHTML : ''
    if (to.getAttribute('inherit') !== 'false') to.appendChild(clone)
  },
...
var app = previousApp || {
  ...
  resetStyles: function () {
    ...
    style.textContent = '*, *::before, *::after { box-sizing:border-box } ' +
      'html { display:flex; height:100%; width:100%; padding-top:env(safe-area-inset-top); padding-bottom:env(safe-area-inset-bottom) }' +
      'body { width:100%; min-height:100%; line-height:1.5; -webkit-font-smoothing:antialiased } ' +
  ...
  load: function () {
    if (!window.frontLoaded) {
      ...
      app.disable(true)
      app.isLocalNetwork = location.protocol === 'file:' || /localhost|127\.0\.0\.1|::1|\.local/i.test(location.hostname)
      app.globals.set('isLocalNetwork', app.isLocalNetwork)
   ...
   */
  globals: {
    frontVersion: { major: 1, minor: 1, patch: 0, build: 811 },
    language: document.documentElement.lang || 'en',
    docMode: document.documentMode || 0,
